...
- `.xlsx`/`.xlsm` attachment means Excel, a URL and `portal` in the body means portal, anything else is skipped.



### 2. **Acknowledge**

- Before any processing, reply with `Re: <subject>` containing a `[ref:<msg_id>]` marker. 
- All state is derived from the mailbox using these markers: "acknowledged" if `/sent` has a mail with the ref, "completed" if that mail's subject starts with `Completed:`. 
- No database or local state file. 
- Mid-run arrivals are also acknowledged via the `ack_pending` callback at ~10% progress intervals.



### 3. **Ingest**

- Excel: render workbook to text, Gemini infers the sheet/table schema (with validation retries), then extract questions. 
- Portal: email the access desk for the current password, log in via Playwright, parse questions from HTML, close the browser.



### 4. **Answer**

- For each question, retrieve top-k KB chunks, generate a structured JSON answer via Ollama (status, confidence, answer, citations, plus extra workbook columns), validate citations and canonicalize enum values.



### 5. **Deliver**

- Excel: write answers into a copy of the original workbook, append a summary sheet, email back as `Completed: <subject>` with the file attached.
- Portal: open a new browser, login with a fresh password, submit the form, send `Completed: <subject>` (no attachment). 
- Failed runs go to `failure.json` and are not retried in the same session.
...
tests/                 Test Excel files + Results
...
4. **Extract** questions and answerable columns per table with `stop_before` boundaries to prevent bleed across stacked tables. Keys are `sheet!table_id!row`. Column wise asnwer patterns stored in `allowed_values` and `rules`
...
**Model used** : `gemini-3.5-flash-lite` (Initial plan was to use `gemini-3.5-flash` but API limitations forced this downgrade)

**Limitations**: Hallucination in terms of row count especially in large files i.e. it does not demand more rows via `needs_full_render=true`. So, for demo and testing, needs_full_render is set to true by default.

### Testing Data:

- Six excel files were generated manually by altering the provided `Industry Standard Questionnaires.xlsx` and including itself for depicting multiple scenarios like: id-question mismatch, empty rows in question column, multiple tables in single sheet, no workbook level instructions, no headers etc.
- For each file, schema of the workbook was generated 5 times to identify recurring issues and evaluate consistency.
Main Schema Inconsistencies:
- The provided table has an `Applicability(Y/N)` column however, the column-level validation allows to choose from **[Yes | No | Partial | Not Applicable]**. The allowed_values for this columns are not constant in each generation.
Amongst the six text files, five contain this column. During the five tests, the results were:
- **[Yes | No | Partial | Applicable]**  had 10 occurrences.
